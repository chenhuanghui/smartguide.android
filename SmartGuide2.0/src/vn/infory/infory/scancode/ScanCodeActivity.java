...
public class ScanCodeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA)) {
			CyUtils.showError("Thiết bị không hỗ trợ camera", null, this);
			return;
		}
		
		mPreview.setPreviewCallback(new PreviewCallback() {
			@Override
			public void onPreviewFrame(byte[] data, Camera camera) {
				...
				// Call scan code api
				ScanCode scanCodeTask = new ScanCode(ScanCodeActivity.this, mQRCode) {
					@Override
...
//		mCamera.lock();
	...
	private void onFlashClick(View v) {
		...
		if (!getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_FLASH)) {
			CyUtils.showError("Thiết bị không hỗ trợ đèn flash", null, this);
			return;
		}
	...
	private void processScanResponse(ScanResponse response) {
		...
		final Dialog dlg = new Dialog(ScanCodeActivity.this,
				android.R.style.Theme_Translucent_NoTitleBar);
		View v = getLayoutInflater().inflate(R.layout.scan_dlg, null);
		...
		View btnContinue = v.findViewById(R.id.btnContinue);
		...
		case 0: {
			...
			View childView = getLayoutInflater().inflate(R.layout.scan_error, layoutContentHolder, true);
		...
		case 2: {
			...
			View childView = getLayoutInflater().inflate(R.layout.scan_sgp_point, layoutContentHolder, true);
		...
		case 3: {
			...
			View childView = getLayoutInflater().inflate(R.layout.scan_sgp_reward, layoutContentHolder, true);
		...
		case 4: {
			...
			View childView = getLayoutInflater().inflate(R.layout.scan_voucher, layoutContentHolder, true);
